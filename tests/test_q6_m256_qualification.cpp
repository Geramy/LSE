// Host-only selection contracts for the isolated M256 model qualification.
#include "harness.hpp"
...
LSE_TEST(m256_qualification_preserves_other_shape_selection) {
  unsetenv("LSE_WMMA");
...
LSE_TEST(m256_qualification_rejects_unmeasured_and_ineligible_requests) {
  for(bool loom:{false,true})for(int reason=0;reason<11;++reason){
    Fixture f(256,reason==0?19:17408,5120,loom);
